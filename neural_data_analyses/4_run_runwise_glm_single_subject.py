...
for run in ['01', '02', '03', '04', '05', '06', '07', '08']:
    ...
    # Modify events to take window of eye contrast, remove for standard analyses
    for i_run, events_run in enumerate(models_events[0]):
        updated_events = []
        for idx, row in events_run.iterrows():
            row = row.copy()
            if str(row["trial_type"]) in ["1", "2", "3", "4", 1, 2, 3, 4]:
                row["duration"] = 1
    ...
    for model in models:
        save_glm_to_bids(
            ...
            out_dir=run_dir,
            prefix=f"sub-{sub}_run-{run}_space-T1w{append_str}",
        )

        print(f"this many residuals for model in {run}: ", len(model.residuals))
        ...
        # save residuals
        # residuals_nii = nib.Nifti1Image(residuals, image_nii.affine, image_nii.header)

        print('residuals affine: ', residuals.affine)
        print('residuals header: ', residuals.header)

        # save residuals
        nib.save(residuals, f'{run_dir}sub-{sub}_run-{run}_space-T1w{append_str}_residuals.nii.gz')
