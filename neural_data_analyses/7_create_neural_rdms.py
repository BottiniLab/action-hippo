...
# last minute addition to account for saved data

roi_data_dir = os.path.join(glm_results_dir, sub, "roi_data")
roi_data_file = os.path.join(
    roi_data_dir,
    f"{sub}{append_str}_{region}-{hemisphere}_roi_data.pkl"
)

if os.path.exists(roi_data_file):
    print(f"Loading precomputed ROI data from: {roi_data_file}")
    with open(roi_data_file, "rb") as f:
        roi_data = pickle.load(f)

    measurements = roi_data["measurements"]
    conds = roi_data["conds"]
    sessions = roi_data["sessions"]
    mask_file_used = roi_data["mask_file"]

    print("Loaded measurements shape:", measurements.shape)
else:
    print("No precomputed ROI data found, extracting from beta maps.")

    # load the mask 
    mask = nib.load(mask_dir + mask_file)

    n_rep = 8
    conds = np.array(['cond_' + x for x in conditions])
    conds = np.repeat(conds, n_rep)

    sessions = np.tile(np.arange(n_rep), len(conditions))
    sessions = np.array(['session_' + str(x+1) for x in sessions])
