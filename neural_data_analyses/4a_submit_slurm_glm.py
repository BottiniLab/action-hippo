...
for sub_id in sub_ids:
    ...
    with open (dir_code+f'sub-{sub_id}_run-glm_contrast-{contrast_}.sh', 'w') as f_glm:
        f_glm.write(f'''\
#! /bin/bash
/data/u_eperon_software/anaconda3/envs/act_hippo/bin/python /data/pt_02747/action_hippo/code/4_run_glm_single_subject.py {sub_id} {contrast_}

''')
        

    # Write slurm file
    with open (dir_code+f'sub-{sub_id}_run-glm_contrast-{contrast_}.slurm', 'w') as f_slurm:
        f_slurm.write(f'''\
#!/bin/bash
...
srun {dir_code}sub-{sub_id}_run-glm_contrast-{contrast_}.sh
''')
        

    # submit job to condor
    os.system(f'''
        chmod a+rwx {dir_code}sub-{sub_id}_run-glm_contrast-{contrast_}.sh
        chmod a+rwx {dir_code}sub-{sub_id}_run-glm_contrast-{contrast_}.slurm
        sbatch {dir_code}sub-{sub_id}_run-glm_contrast-{contrast_}.slurm
        ''')
